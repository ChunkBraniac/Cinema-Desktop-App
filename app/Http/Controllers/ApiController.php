...
class ApiController extends Controller
{
    ...
    public function latest()
    {
        ...
        $apiKey = '1188d67425fbb7eab235eb038542b64f';
        $today = date('Y-m-d');

        $curl = curl_init();

        curl_setopt_array($curl, [
            CURLOPT_URL => "https://api.themoviedb.org/3/discover/movie?api_key=$apiKey&primary_release_date.gte=$today&primary_release_date.lte=$today",
            CURLOPT_RETURNTRANSFER => true,
            CURLOPT_ENCODING => "",
            CURLOPT_MAXREDIRS => 10,
            CURLOPT_TIMEOUT => 30,
        ...
        if ($err) {
            ...
            if (isset($data['results'])) {
                foreach ($data['results'] as $movie) {
                    $id = $movie['id'];
                    $release_date = mysqli_real_escape_string($connection, $movie['release_date']);
                    $year = substr($release_date, 0, 4); // Extract the first four characters

                    $fetch = mysqli_query($connection, "SELECT * FROM latests WHERE movieId = '$id'");
                    if (mysqli_num_rows($fetch) == 0) {
                        $adult = $movie['adult'];
                        $backdrop_path = isset($movie['backdrop_path']) ? mysqli_real_escape_string($connection, $movie['backdrop_path']) : 0;
                        $language = strtoupper($movie['original_language']);
                        $name = mysqli_real_escape_string($connection, $movie['title'] . ' ' . $year);
                        $overview = mysqli_real_escape_string($connection, $movie['overview']);
                        $poster_path = mysqli_real_escape_string($connection, $movie['poster_path']);
                        $vote_average = mysqli_real_escape_string($connection, $movie['vote_average']);

                        $base_url = "https://image.tmdb.org/t/p/w500" . $poster_path;
                        $formatted_name = str_replace(' ', '-', $name);
                        $rating = floor($vote_average * 10) / 10;

                        $insert = mysqli_query($connection, "INSERT INTO latests (movieId, isAdult, originalTitleText, imageUrl, backdrop_path, country, language, plotText, releaseDate, releaseYear, aggregateRating, titleType, runtime, genres, trailer, created_at) VALUES ('$id', '$adult', '$formatted_name', '$base_url', '$backdrop_path', '', '$language', '$overview', '$release_date', '$year', '$rating', 'movie', '', '', '', now())");
                ...
                echo "No movies found for today.\n";
            }
        }
    }
