...
class ApiController extends Controller
{
                    ...
                    if ($year >= '2020' && $year <= '2024') {
                        ...
                        if (mysqli_num_rows($fetch) == 0) {
                            ...
                            if (mysqli_num_rows($fetch_latest) == 0) {
                                ...
                                $insert_latest = mysqli_query($connection, "INSERT INTO series (movieId, isAdult, originalTitleText, imageUrl, backdrop_path, country, language, plotText, releaseDate, releaseYear, aggregateRating, runtime, genres, trailer, titleType, created_at) VALUES ('$id', '$adult', '$formatted_name', '$base_url', '$backdrop_path', '$country', '$language', '$overview', '$first_air_date', '$year', '$rating', '', '', '', 'series', now())");

                                if ($insert_latest) {
                                    echo "Series added to latest successfully";
                                } else {
                                    echo "Failed to add series";
                                }

                            } else {
                                echo "Series already in latest series table";
                            }
                        } else {
    ...
    public function updateLatestMoviesInfo()
    {
        $dbhost = "127.0.0.1";
        $dbus = "root";
        $dbpass = '';
        ...
        $fetch = mysqli_query($connection, "SELECT * FROM latests WHERE titleType = 'movie'");
        ...
        if (mysqli_num_rows($fetch) > 0) {
            while ($movie = mysqli_fetch_assoc($fetch)) {
                ...
                // update the movie in the database
                $update = mysqli_query($connection, "UPDATE latests SET genres = '$genres', country = '$origin_country', runtime = '$runtime' WHERE id = '$movieId'");
    ...
    public function updateLatestSeriesInfo()
    {
        $dbhost = "127.0.0.1";
        $dbus = "root";
        $dbpass = '';
        ...
        $fetch = mysqli_query($connection, "SELECT * FROM latests WHERE titleType = 'series'");
        ...
        if (mysqli_num_rows($fetch) > 0) {
            while ($movie = mysqli_fetch_assoc($fetch)) {
                ...
                // update the movie in the database
                $update = mysqli_query($connection, "UPDATE latests SET genres = '$genres' WHERE id = '$movieId'");
    ...
    public function updateLatestTrailer()
    {
        $dbhost = "127.0.0.1";
        $dbus = "root";
        $dbpass = '';
        ...
        $fetch_trailer = mysqli_query($connection, "SELECT * FROM latests WHERE trailer = ''");
        ...
        if (mysqli_num_rows($fetch_trailer) > 0) {
            ...
            // Fetch movies that don't have trailers
            $noTrailer = mysqli_query($connection, "SELECT * FROM latests WHERE trailer = ''");
            ...
            if (mysqli_num_rows($noTrailer) > 0) {
                while ($nonTrailer = mysqli_fetch_assoc($noTrailer)) {
                    ...
                    foreach ($response['items'] as $item) {
                        ...
                        // Update the database with the trailer
                        $updateTrailer = mysqli_query($connection, "UPDATE latests SET trailer = '$videoTrailer' WHERE originalTitleText = '$movieName'");
                        if ($updateTrailer) {
                            // return redirect()->route('admin.dashboard')->with('success', 'Trailer updated');
                            echo "Trailer updated";
                        } else {
    ...
    public function latestSeries()
    {
        ...
        curl_setopt_array($curl, [
            CURLOPT_URL => "https://api.themoviedb.org/3/discover/tv?first_air_date.gte={$today}&first_air_date.lte={$today}",
            CURLOPT_RETURNTRANSFER => true,
            CURLOPT_ENCODING => "",
            CURLOPT_MAXREDIRS => 10,
            CURLOPT_TIMEOUT => 30,
        ...
        if (isset($data['results']) && is_array($data['results'])) {
            foreach ($data['results'] as $results) {
                ...
                // check if the series is already in the db
                $fetch = mysqli_query($connection, "SELECT * FROM latests WHERE movieId = '$id'");

                if (mysqli_num_rows($fetch) == 0) {
                    $adult = $results['adult'];
                    $backdrop_path = isset($results['backdrop_path']) ? mysqli_real_escape_string($connection, $results['backdrop_path']) : 0;
                    $origin_country = isset($results['origin_country'][0]) ? $results['origin_country'][0] : 0;
                    $language = strtoupper($results['original_language']);
                    $name = mysqli_real_escape_string($connection, $results['name'] . ' ' . $id);
                    ...
                    $vote_average = mysqli_real_escape_string($connection, $results['vote_average']);
                    ...
                    // insert into the series table
                    $insert = mysqli_query($connection, "INSERT INTO latests (movieId, isAdult, originalTitleText, imageUrl, backdrop_path, country, language, plotText, releaseDate, releaseYear, aggregateRating, runtime, genres, trailer, titleType, created_at) VALUES ('$id', '$adult', '$formatted_name', '$base_url', '$backdrop_path', '$origin_country', '$language', '$overview', '$first_air_date', '$year', '$rating', '', '', '', 'series', now())");
    ...
    public function latestMovies()
    {
        ...
        curl_setopt_array($curl, [
            CURLOPT_URL => "https://api.themoviedb.org/3/discover/movie?primary_release_date.gte=$today&primary_release_date.lte=$today",
            CURLOPT_RETURNTRANSFER => true,
            CURLOPT_ENCODING => "",
            CURLOPT_MAXREDIRS => 10,
            CURLOPT_TIMEOUT => 30,
        ...
        if (isset($data['results']) && is_array($data['results'])) {
            foreach ($data['results'] as $results) {
                ...
                if (mysqli_num_rows($fetch) == 0) {
                    ...
                    $backdrop_path = isset($results['backdrop_path']) ? mysqli_real_escape_string($connection, $results['backdrop_path']) : 0;
                    $language = strtoupper($results['original_language']);
                    $name = mysqli_real_escape_string($connection, $results['title'] . ' ' . $id);
                    $overview = mysqli_real_escape_string($connection, $results['overview']);
                    $poster_path = mysqli_real_escape_string($connection, $results['poster_path']);
                    $vote_average = mysqli_real_escape_string($connection, $results['vote_average']);
                    ...
                    // insert into the series table
                    $insert = mysqli_query($connection, "INSERT INTO latests (movieId, isAdult, originalTitleText, imageUrl, backdrop_path, country, language, plotText, releaseDate, releaseYear, aggregateRating, runtime, genres, trailer, titleType, created_at) VALUES ('$id', '$adult', '$formatted_name', '$base_url', '$backdrop_path', '', '$language', '$overview', '$release_date', '$year', '$rating', '', '', '', 'movie', now())");
