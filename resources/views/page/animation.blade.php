            ...
            @unless (count($paginatedResults) == 0)
                @foreach ($paginatedResults as $animation)
                    <div class="col-6 col-sm-4 col-md-3 col-lg-2 col-xl-2 mt-3">
                        ...
                        <a href="{{ url('media/' . $animation->originalTitleText . '/' . $animation->titleType) }}" class="text-decoration-none text-dark" data-bs-toggle="tooltip" data-bs-placement="bottom" data-bs-title="{{ $animation->originalTitleText . ' ' . '(' . $animation->releaseYear . ')' }}">
                            <h6 class="mt-1 text-truncate" style="font-family: 'Ubuntu sans', sans-serif; font-weight: 500">{{ $animation->originalTitleText . ' ' . '(' . $animation->releaseYear . ')' }}</h6>
                        </a>
